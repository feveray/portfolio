...
export default function CardTags() {
  ...
  return (
    <div className="w-full h-72 m-8 bg-[#AABF91] p-4 flex flex-col rounded-xl shadow-[-20px_20px_0px_-5px_#31572c] text-[#31572c]">
      <span className="font-black text-2xl mb-4">Hard skills</span>
      <div className="overflow-auto h-full">
        <ul className="flex flex-wrap">
          {tags.map((tag, index) => (
            <li
              key={index}
              className="relative inline-block text-white text-lg bg-[#31572c] px-6 py-2 rounded-full m-2 cursor-pointer transition-transform duration-300 hover:scale-110 hover:bg-[#51572c]
